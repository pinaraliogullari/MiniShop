...
namespace MiniShop.UI.Controllers
{
    public class OrderController : Controller
    {
        ...
        public async Task<IActionResult> CheckOut(OrderViewModel orderViewModel)
        {
            ...
            options.ApiKey = "sandbox-BFBhhNpcR6bwpVPyqY0efn2lfENN5ACI";
            options.SecretKey = "sandbox-WwkKblIUbmjMKcpGz22OnGarbK2MRPbd";
            ...
            if (payment.Status == "success")
            {
                Order order = new Order
                {
                    ...
                    OrderDetails = orderViewModel.ShoppingCart.ShoppingCartItems.Select(x => new OrderDetail
                    {
                     ProductId = x.ProductId,
                     Price=x.ProductPrice,//sipariş oluşunca productın priceını orderdetaildeki price a atıyoruz. bu propertyi oluişturma amacımız şuydu; biz productın priceını değiştirsek bile order detailin price ı etkilenmeyecek.
                     Quantity = x.Quantity,

                    }).ToList()
                };
                await _orderManager.CreateAsync(order);
                await _shoppingCartManager.ClearShoppingCartAsync(orderViewModel.ShoppingCart.Id);
                return Redirect("~/");
            }
